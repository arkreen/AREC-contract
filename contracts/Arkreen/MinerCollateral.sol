...
contract MinerCollateral is OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => StakeInfo) public stakeInfo;             // Depositor -> deposit Info
    ...
    event DepsitCollateral(uint256 indexed txid, address indexed staker, address cspminer, uint256 amount);
    event WithdrawCollateral(uint256 indexed txid, address indexed staker, address cspminer, uint256 amount, uint256 reward);
    event SlashCollateral(uint256 indexed txid, address cspminer, address owner, uint256 amount);
    ...
    function stake(uint256 txid, address cspminer, uint256 amount, uint256 nonce, uint256 deadline, Sig calldata signature) external nonReentrant ensure(deadline) {
        ...
        emit DepsitCollateral(txid, msg.sender, cspminer, amount);
    }
    ...
    function unstakeWithReward(uint256 txid, address cspminer, uint256 amount, uint256 reward, uint256 nonce, uint256 deadline, Sig calldata signature) external nonReentrant ensure(deadline){
        ...
        emit WithdrawCollateral(txid, msg.sender, cspminer, amount, reward);
    }
    ...
    function stakeSlash(uint256 txid, address cspminer, address owner, uint256 amount) external {
        ...
        minerStakeInfo[cspminer].amountStake -= uint96(amount); 
        totalStake = totalStake - uint96(amount);
        emit SlashCollateral(txid, cspminer, owner, amount);
    }
}