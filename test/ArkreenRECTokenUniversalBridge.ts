...
*/
    it("Offset Details: 18 AREC with bridge, Start with Bridged REC", async () => {     // Maximum 20 NFT in one offset transaction
      ...
      await mintARECIREC(2000)          // 8:   170
      await mintARECIREC(9000)          // 9:   260
      await mintARECIREC(800)           // 10；  268
      await mintARECIREC(3000)          // 11:  298
      await mintARECIREC(20000)     // 12

      await mintARECIREC(5000)          // 13:  348
      await mintARECIREC(600)           // 14:  354
      await mintARECIREC(8000)          // 15:  434
      await mintARECIREC(500)           // 16:  439
      await mintARECIREC(1000)          // 17:  449
      await mintARECIREC(2000)          // 18:  469
      await mintARECIREC(9000)          // 19:  559
      await mintARECIREC(800)           // 20:  567
      await mintARECIREC(3000)          // 21： 597
      await mintARECIREC(20000)     // 22
      await mintARECIREC(200)       // 23:  691

      const balance_1 = await arkreenRECToken.balanceOf(owner1.address)
      await arkreenRECToken.connect(owner1).commitOffset(expandTo9Decimals(1000))

      expect(await arkreenBadge.getBridgeDetailStatus(arkreenRECToken.address)).to.deep.eq([BigNumber.from(0), BigNumber.from(0)])
      expect(await arkreenBadge.getDetailStatus(arkreenRECToken.address)).to.deep.eq([expandTo9Decimals(0), BigNumber.from(1)])

      const balance_1_A = await arkreenRECToken.balanceOf(owner1.address)
      expect(balance_1_A).to.equal(balance_1.sub(expandTo9Decimals(1000)))

      await expect(arkreenRECToken.connect(owner1).commitOffset(expandTo9Decimals(13600)))
              .to.emit(arkreenRECToken, "OffsetFinished")
              .withArgs(owner1.address, expandTo9Decimals(13600), 2) 
