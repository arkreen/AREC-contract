...
describe("GreenBTC2 Test Campaign", ()=>{
    ...
    describe('GreenBTC2 test', () => {
      ...
      it("GreenBTC2 basics test", async function () {
        ...
        const claimActionGiftsTx = await greenBTC2.claimActionGifts(1, blockHeight, blockHash, {v,r,s})
        const claimReceipt = await claimActionGiftsTx.wait()

        //console.log("JJJJJJJJJJJJJ", claimReceipt);


        const amount1 = await greenBTCGift.balanceOf(deployer.address, 1)
        const amount2 = await greenBTCGift.balanceOf(deployer.address, 2)
        const amount3 = await greenBTCGift.balanceOf(deployer.address, 3)
        ...
        const claimGiftTX = await greenBTCGift.claimGift(3, BigNumber.from(2))
...
//        console.log("MMMMMMMMMMMMMMM", claimMultiGiftsReceipt);
      ...
      it("GreenBTC2 GreenBTCGift+ClaimGift Test", async function () {
        ...
        await mine(5)
        await greenBTC2.claimActionGifts(1, receipt.blockNumber, receipt.blockHash, {v,r,s})

        const amount3 = await greenBTCGift.balanceOf(deployer.address, 3)
        const balanace1 = await AKREToken.balanceOf(deployer.address)
        const unitGift = expandTo18Decimals(100)

        await expect(greenBTCGift.claimGift(5, amount3))
                .to.be.revertedWith("GBTC: Wrong Gift ID")

        expect(await greenBTCGift.claimGift(3, BigNumber.from(2)))
                .to.emit(tokenA, 'GiftClaimed')
                .withArgs(deployer.address, 3, BigNumber.from(2))

        expect(await greenBTCGift.balanceOf(deployer.address, 3)).to.eq(amount3.sub(BigNumber.from(2)))
        expect(await AKREToken.balanceOf(deployer.address)).to.eq(balanace1.add(unitGift.mul(2)))

        await greenBTCGift.claimGift(3, amount3.sub(BigNumber.from(2)))

        expect(await greenBTCGift.balanceOf(deployer.address, 3)).to.eq(0)
        expect(await AKREToken.balanceOf(deployer.address)).to.eq(balanace1.add(unitGift.mul(amount3)))

        await expect(greenBTCGift.claimGift(3, 1))
                .to.be.revertedWith("ERC1155: burn amount exceeds balance")

        const amount1 = await greenBTCGift.balanceOf(deployer.address, 1)
        const amount2 = await greenBTCGift.balanceOf(deployer.address, 2)

        await greenBTCGift.claimGift(1, amount1)
        await greenBTCGift.claimGift(2, amount2)
        const claimGiftTx = await greenBTCGift.claimGift(3, 0)

        console.log("UUUUUUUUUUUUUUUUUUU", claimGiftTx);
