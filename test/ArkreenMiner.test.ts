...
//        console.log("recoveredAddress, deployer.address", recoveredAddress, deployer.address)
  ...
  describe("ArkreenMiner: Upgrading test", () => {
    async function deployArkreenMinerFixture() {
      ...
      const ArkreenMinerV2 = await upgrades.upgradeProxy(ArkreenMiner.address, ArkreenMinerFactoryV2)
      await ArkreenMinerV2.deployed()    
      return { ArkreenMinerV2 }
    }
    it("ArkreenMiner Upgrading: Basic check", async () => {
      const { ArkreenMinerV2 } = await loadFixture(deployArkreenMinerFixture);
      expect(await ArkreenMinerV2.version()).to.equal('1.0.1');
      const receivers = randomAddresses(10)
      const miners = randomAddresses(10)
      await ArkreenMinerV2.connect(deployer).AirdropMiners(receivers, miners)
      expect(await ArkreenMinerV2.tokenURI(9)).to.equal('https://www.arkreen.com/miners/9');
      await ArkreenMinerV2.connect(deployer).updateMineMore(9, "Miner 9 more testing info")
      expect(await ArkreenMinerV2.getMineMore(9)).to.equal('Miner 9 more testing info');
    })
  })
});
